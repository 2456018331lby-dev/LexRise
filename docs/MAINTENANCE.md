...
**前置**：确认你已经 revoke 了家目录仓库里那个泄露的 `ghp_...` PAT，且项目仓库已有 remote。
...
- 想确认自己在对的仓库：`git remote -v` 应该空（或者指向 LexRise 的 GitHub repo，而不是家目录的 `github` remote）
...
- `git push --force` 到 main（项目没真正协作，意义有限，风险大）
