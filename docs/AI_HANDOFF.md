...
**最近一次更新**：2026-06-09（v0.33 完工后）

---

## 1. 项目一句话描述

`c:/Users/24560/Desktop/study/Englishdemo` 是一个本地离线的 Android 背单词 App（Kotlin + Compose + Material3 dynamicColor + Room v2 + WorkManager），面向 CET4/CET6/考研。当前能跑、148 个 Android 单测 + 3 个 Python 脚本单测全绿，debug APK 可装。
...
- `app/build.gradle.kts` 已同步 `versionCode=33` / `versionName="0.33.0"`

**已做的测试 / 构建验证**：

- `./gradlew.bat :app:lintDebug` — **通过**（1 个既有 `GradleDependency` 版本提示）
- `python -m unittest discover -s tools -p "test_*.py"` — **3 个 test 全绿**
- `./gradlew.bat :app:testDebugUnitTest` — **148 个 test 全绿**
- `./gradlew.bat :app:assembleDebug` — 成功，debug APK 64.69 MB
...
| `MorphologyHelpersTest.kt` | 124 | decomposeWord / computePaceRecommendation / **buildStudyFocusCue** / **buildDailyLoadBrief** / **buildDailyStudyRoute** / **buildStudyRhythmBrief** / **buildRootAtlasBrief** / **buildRootMnemonicBrief** / **buildRootWordGuide** / **buildClozeContextGuide** / **buildWordMemoryAnchor** / **buildWordBatchBrief** / **buildMnemonicBatchBrief** / **buildReviewQueueBrief** / **buildReviewExitBrief** / **buildRootGroupInsight** / **buildPracticeSessionCoach** / **buildToughWordPrescription** / **buildToughWordsBrief** / **buildVocabularySearchInsight** / **buildVocabularySearchRescuePlan** / **buildVocabularyResultTriage** / **buildClozeBlank**（含原词优先、最早派生词优先） / **buildQuizOptions** / **blankTermInExample** / **fuzzyTermMatchDistance** / **fuzzyWordFormMatchDistance** / **matchingWordForms** / **recordPracticeAttempt** / **normalizedEditDistance** / **ratingFromEditDistance** |

Android **总计 148 个单测**。另有 `tools/test_build_wordlists.py` 3 个 Python unittest 覆盖 mnemonic seed/raw 读取与覆盖规则。
...
v0.33 后，以下是还没开工或值得继续深化的方向。按优先级从高到低。
...
2. `./gradlew.bat :app:testDebugUnitTest` 全绿（当前 148 个）
3. `./gradlew.bat :app:assembleDebug` 成功
4. APK 体积 < 80 MB（v0.3 基线 67 MB）
5. 手装 APK 到手机/模拟器，**v0.33 冒烟清单**：
...
## 10. APK 与 GitHub 当前状态（v0.33 验证后）

**APK**  
- 最近一次 `assembleDebug` 验证生成：`app/build/outputs/apk/debug/app-debug.apk`，64.69 MB，未签名 debug 包；清理构建产物后可重跑命令再生成
...
- GitHub v0.33 快照包含 57 个 Git 跟踪文件，不包含 `.android-sdk/`、`local.properties`、`tools/raw/`、`app/build/`、`build/`、`.gradle/`、`.kotlin/`
...
3. `python -m unittest discover -s tools -p "test_*.py"` 确认脚本测试全绿；`./gradlew.bat :app:testDebugUnitTest` 确认当前 148 个 Android 单测全绿
