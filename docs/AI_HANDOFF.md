...
**最近一次更新**：2026-06-09（v0.32 完工后）

---

## 1. 项目一句话描述

`c:/Users/24560/Desktop/study/Englishdemo` 是一个本地离线的 Android 背单词 App（Kotlin + Compose + Material3 dynamicColor + Room v2 + WorkManager），面向 CET4/CET6/考研。当前能跑、141 个 Android 单测 + 3 个 Python 脚本单测全绿，debug APK 可装。
...
- **词汇搜索（v0.6/v0.12/v0.13/v0.20/v0.32）**：直接命中不足时按英文词形相似度补充候选；`clarfy` 这类小 typo 也能找回 `clarify`；v0.12 起直接搜索和 fuzzy fallback 都支持 derivatives，`clarified` / `clarifed` 也能找回 `clarify`；v0.13 起结果卡展示“词形雷达”，说明命中的原词/派生词形；v0.20 起搜索框下方新增“检索洞察”，判断当前是原词命中、词形/typo、同根聚簇、释义命中还是无结果；v0.32 起空结果时新增“空结果救援”，按阶段筛选、英文词干、原形、拼写和中文释义给出改查路线
...
- `app/build.gradle.kts` 已同步 `versionCode=32` / `versionName="0.32.0"`

**已做的测试 / 构建验证**：

- `./gradlew.bat :app:lintDebug` — **通过**（1 个既有 `GradleDependency` 版本提示）
- `python -m unittest discover -s tools -p "test_*.py"` — **3 个 test 全绿**
- `./gradlew.bat :app:testDebugUnitTest` — **141 个 test 全绿**
- `./gradlew.bat :app:assembleDebug` — 成功，debug APK 64.65 MB
...
| `MorphologyHelpersTest.kt` | 117 | decomposeWord / computePaceRecommendation / **buildStudyFocusCue** / **buildDailyLoadBrief** / **buildDailyStudyRoute** / **buildStudyRhythmBrief** / **buildRootAtlasBrief** / **buildRootMnemonicBrief** / **buildRootWordGuide** / **buildClozeContextGuide** / **buildWordMemoryAnchor** / **buildWordBatchBrief** / **buildMnemonicBatchBrief** / **buildReviewQueueBrief** / **buildReviewExitBrief** / **buildRootGroupInsight** / **buildPracticeSessionCoach** / **buildToughWordPrescription** / **buildToughWordsBrief** / **buildVocabularySearchInsight** / **buildVocabularySearchRescuePlan** / **buildClozeBlank**（含原词优先、最早派生词优先） / **buildQuizOptions** / **blankTermInExample** / **fuzzyTermMatchDistance** / **fuzzyWordFormMatchDistance** / **matchingWordForms** / **recordPracticeAttempt** / **normalizedEditDistance** / **ratingFromEditDistance** |

Android **总计 141 个单测**。另有 `tools/test_build_wordlists.py` 3 个 Python unittest 覆盖 mnemonic seed/raw 读取与覆盖规则。
...
v0.32 后，以下是还没开工或值得继续深化的方向。按优先级从高到低。
...
2. `./gradlew.bat :app:testDebugUnitTest` 全绿（当前 141 个）
3. `./gradlew.bat :app:assembleDebug` 成功
4. APK 体积 < 80 MB（v0.3 基线 67 MB）
5. 手装 APK 到手机/模拟器，**v0.32 冒烟清单**：
...
## 10. APK 与 GitHub 当前状态（v0.32 GitHub API 快照后）

**APK**  
- 最近一次 `assembleDebug` 验证生成：`app/build/outputs/apk/debug/app-debug.apk`，64.65 MB，未签名 debug 包；清理构建产物后可重跑命令再生成
...
3. `python -m unittest discover -s tools -p "test_*.py"` 确认脚本测试全绿；`./gradlew.bat :app:testDebugUnitTest` 确认当前 141 个 Android 单测全绿
