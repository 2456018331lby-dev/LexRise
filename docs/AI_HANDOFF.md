...
**最近一次更新**：2026-06-09（v0.27 完工后）

---

## 1. 项目一句话描述

`c:/Users/24560/Desktop/study/Englishdemo` 是一个本地离线的 Android 背单词 App（Kotlin + Compose + Material3 dynamicColor + Room v2 + WorkManager），面向 CET4/CET6/考研。当前能跑、113 个 Android 单测 + 3 个 Python 脚本单测全绿，debug APK 可装。
...
- `app/build.gradle.kts` 已同步 `versionCode=27` / `versionName="0.27.0"`

**已做的测试 / 构建验证**：

- `./gradlew.bat :app:lintDebug` — **通过**（1 个既有 `GradleDependency` 版本提示）
- `python -m unittest discover -s tools -p "test_*.py"` — **3 个 test 全绿**
- `./gradlew.bat :app:testDebugUnitTest` — **113 个 test 全绿**
- `./gradlew.bat :app:assembleDebug` — 成功，debug APK 64.46 MB
...
Android **总计 113 个单测**。另有 `tools/test_build_wordlists.py` 3 个 Python unittest 覆盖 mnemonic seed/raw 读取与覆盖规则。
...
v0.27 后，以下是还没开工或值得继续深化的方向。按优先级从高到低。
...
2. `./gradlew.bat :app:testDebugUnitTest` 全绿（当前 113 个）
3. `./gradlew.bat :app:assembleDebug` 成功
4. APK 体积 < 80 MB（v0.3 基线 67 MB）
5. 手装 APK 到手机/模拟器，**v0.27 冒烟清单**：
...
## 10. APK 与 GitHub 当前状态（v0.27 后）

**APK**  
- 最近一次 `assembleDebug` 验证生成：`app/build/outputs/apk/debug/app-debug.apk`，64.46 MB，未签名 debug 包；清理构建产物后可重跑命令再生成
...
3. `python -m unittest discover -s tools -p "test_*.py"` 确认脚本测试全绿；`./gradlew.bat :app:testDebugUnitTest` 确认当前 113 个 Android 单测全绿
