...
- **完形模式二期：多句例句质量排序**
  - 完形题现在会先把导入词书里的多句例句切成候选句，再选择更适合训练的那一句挖空
  - 评分同时考虑命中原词/派生词、句长、空格位置和句末标点，优先给出短而完整、空格不过分靠边的题干
  - 单句内仍保留旧规则：原词优先；原词缺失时按例句中最早出现的派生词优先，正确答案继续保留例句里的实际词形
  - 逻辑由 `buildClozeBlank` 纯函数派生，`buildClozeQuestion` 通过原接口自然复用，不新增数据库字段，不改变 Review UI、选项排序或 SRS 评分
...
- **词汇搜索新增“空结果救援”**
  - 当搜索没有结果时，在“检索洞察”下方新增救援卡，按顺序提示取消阶段筛选、补足英文词干、换回原形、缩短词干、校准拼写或换中文近义释义
  - 阶段筛选处于“新词 / 学习中 / 复习 / 掌握”时，会优先提示先切回“全部阶段”，避免用户误以为词书没有这个词
  - 英文短查询、派生词查询、中文释义查询分别给不同建议，让空结果页从“换个关键词”升级为可执行的检索路线
  - 逻辑由 `buildVocabularySearchRescuePlan` 纯函数派生，不新增数据库字段，不改变搜索排序、阶段筛选、词形容错或用户设置

## v0.31 亮点

- **Review Tab 新增“本轮收口建议”**
  - 在“复习队列预案”和“本轮练习仪表”之间新增收口卡，练习过程中持续判断是否应先开始、继续本组、转修错题、加难继续、直接收尾或本轮收口
  - 结合本轮完成数、稳答数、再练数、稳定率、当前练习模式和剩余到期词数，让用户不只看到统计，还知道什么时候该停、该加难还是该修复
  - 卡片展示行动标签、两项关键指标和进度条，沿用 Review 页渐变卡视觉，不新增数据库字段、不改 SRS 评分、不改变复习队列顺序
  - 逻辑由 `buildReviewExitBrief` 纯函数派生，覆盖 6 个分支单测
...
- `buildClozeBlank` 保持原 API 和单句内原词/派生词优先级，`buildClozeQuestion` 自动复用更优候选句
- 新增多句例句和 Repository 构题回归测试，覆盖导入词书的完形题质量
- 应用版本元数据同步到 0.36.0
- Android 单测 159 → 162
...
- 新增 `VocabularySearchRescuePlan` / `VocabularySearchRescueStep` 和 `buildVocabularySearchRescuePlan` 纯函数
- 新增 `VocabularySearchRescueCard`，展示行动标签、救援强度和最多 4 条改查路线
- 应用版本元数据同步到 0.32.0
- Android 单测 135 → 141

### v0.31（2026-06-09）
- Review Tab 新增“本轮收口建议”，按本轮稳定率、再练数、当前模式和剩余到期词判断开始、继续、修复、加难、收尾或清空状态
- 新增 `ReviewExitBrief` / `ReviewExitBriefKind` 和 `buildReviewExitBrief` 纯函数
- 新增 `ReviewExitBriefCard`，展示行动标签、进度条和两项收口指标
- 应用版本元数据同步到 0.31.0
- Android 单测 129 → 135
